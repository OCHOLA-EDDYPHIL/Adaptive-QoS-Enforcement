...
model_path = "/models/dtree_model3.pkl"
...
flow_lock = Lock()  # Lock for thread-safe access to shared flow data
MIN_PACKETS = 10
...
def classify_flow(key, flow):
    ...
    def stats(pkts):
        ...
        return len(sizes), sum(sizes), sum(sizes) / len(sizes) if sizes else 0, sum(iats) / len(iats) if iats else 0

    fwd_cnt, fwd_bytes, fwd_mean_size, fwd_iat = stats(fwd)
    bwd_cnt, bwd_bytes, bwd_mean_size, bwd_iat = stats(bwd)
    ...
    try:
        row = {
            ...
            "FwdPktLenMean": fwd_mean_size, "BwdPktLenMean": bwd_mean_size,
        ...
        confidence, label_idx = max(proba), proba.argmax()
        label = label_encoder.inverse_transform([label_idx])[0]
        if confidence < 0.6: label = "unknown"

        dscp = DSCP_MAP[label]
        print(f"[+] Flow {src_ip}:{src_port} → {dst_ip}:{dst_port} classified as {label} ({confidence:.2f}) → DSCP {dscp}")

        for s, d in [(src_ip, dst_ip), (dst_ip, src_ip)]:
            if not rule_exists(s, d, dscp):
                subprocess.run(["iptables", "-t", "mangle", "-A", "PREROUTING", "-s", s, "-d", d, "-j", "DSCP", "--set-dscp", str(dscp)])
